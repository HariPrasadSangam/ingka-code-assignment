...
public class WarehouseResourceTest {
    ...
    public void testReplaceTheCurrentActiveWarehouseBadRequest() {
        ...
        warehouse.setCapacity(1); // Too small to accommodate existing stock of 27
        warehouse.setStock(1); // Stock doesn't exceed capacity, but capacity can't accommodate existing stock

        given()
                .contentType("application/json")
                .body(warehouse)
                .when().post(path + "/MWH.023/replacement")
                .then()
                .statusCode(400);
    }*/
}
